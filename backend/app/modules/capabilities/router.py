...
router = APIRouter(prefix="/api/v1/webhooks", tags=["benefit-entitlements"])


class BenefitEntitlementWebhookPayload(BaseModel):
...
@router.post("/benefit-entitlements")
async def benefit_entitlement_webhook(
    request: Request,
    db: DBSession,
    ...
    try:
        payload = BenefitEntitlementWebhookPayload.model_validate_json(body)
    ...
    return {
        ...
        "course_entitlements_changed": result.course_entitlements_changed,
